...
//      }
    ...
    
    {
      ...
      findNonZero(filledContourBinMat, locations);
      ...
      for ( Point2i p : locations ) {
        ...
        int y = p.y;
        Coord c(x, y);
        filledContourCoords.push_back(c);
      }
    }
    ...
    if (debugDumpImages) {
      ...
      writeWroteImg(fname, filledContourBinMat);
      cout << "" << endl;
    }
...
//    unordered_map<int, vector<Coord>> edgesOutsideMap;
    ...
    while (!done) {
      ...
      if (outsideStep == 0) {
        ...
        for ( Coord c : outExpandedCoords ) {
          uint8_t isOn = filledContourBinMat.at<uint8_t>(c.y, c.x);
          if ( isOn ) {
