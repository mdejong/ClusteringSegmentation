...
//      }
    ...
    // This Mat is initially the filled region pixels, it is then expanded
    // in steps.
    
    Mat regionBinMat(inputImg.size(), CV_8UC1, Scalar(0));
    drawOneContour(regionBinMat, contour, Scalar(0xFF), CV_FILLED, 8);
    
    if (debugDumpImages) {
      std::stringstream fnameStream;
      fnameStream << "srm" << "_tag_" << tag << "_hull_iter_outside_fill_color" << ".png";
      string fname = fnameStream.str();
      
      writeWroteImg(fname, regionBinMat);
      cout << "" << endl;
    }
    ...
    while (!done) {
      ...
      {
        // The set of points generated by the outgoing vectors may contain
        // some gaps.
        
        // FIXME: this non-optimal method of passing all the region coords could be reworked as
        // a version of the method that accepts a Mat directly to avoid the conversion.
        
        vector<Coord> filledContourCoords;
        
        {
          vector<Point2i> locations;
          findNonZero(regionBinMat, locations);
          assert(locations.size() > 0);
          filledContourCoords.reserve(locations.size());
          
          for ( Point2i p : locations ) {
            filledContourCoords.push_back(pointToCoord(p));
          }
        }
        
        int numToExpand = 1;
        vector<Coord> outExpandedCoords;
        ...
        if (debugDumpInsideOutsiteExpandStepImages) {
          ...
          std::stringstream fnameStream;
          fnameStream << "srm" << "_tag_" << tag << "_hull_iter_outside_expand" << outsideStep << ".png";
          string fname = fnameStream.str();
        ...
        vector<Coord> activatedCoords;
        activatedCoords.reserve(outExpandedCoords.size());
        
        filter(outExpandedCoords, activatedCoords, [&regionBinMat](const Coord & c)->bool {
          uint8_t wasOn = regionBinMat.at<uint8_t>(c.y, c.x);
          if (wasOn) {
            // Ignore coords that were on previously.
            return false;
          } else {
            return true;
          }
        ...
        // Set any activated coords back in regionBinMat, for next expand loop
        ...
        if (debugDumpInsideOutsiteExpandStepImages) {
          ...
          std::stringstream fnameStream;
          fnameStream << "srm" << "_tag_" << tag << "_hull_iter_outside_expand" << outsideStep << "_activated.png";
          string fname = fnameStream.str();
        ...
        // Scan activated coords and split into different vectors by type
        
        vector<Coord> activatedNormalVectorCoords;
        vector<Coord> activatedInBetweenCoords;
        ...
        for ( Coord c : activatedCoords ) {
          ...
          if (pixel == 0xFFFFFF) {
            ...
            // Should not activate inside contour pixel
            assert(0);
          } else if (pixel == 0x0) {
            // No pixel should be off
            assert(0);
        ...
        if (debugDumpInsideOutsiteExpandStepImages) {
          ...
          std::stringstream fnameStream;
          fnameStream << "srm" << "_tag_" << tag << "_hull_iter_outside_expand" << outsideStep << "_activated_normal.png";
          string fname = fnameStream.str();
        ...
        if (debugDumpInsideOutsiteExpandStepImages) {
          ...
          std::stringstream fnameStream;
          fnameStream << "srm" << "_tag_" << tag << "_hull_iter_outside_expand" << outsideStep << "_activated_inbetween.png";
          string fname = fnameStream.str();
      ...
      if (!done)
      {
        ...
        for ( int contouri = 0; contouri < maxContouri; contouri++ ) {
          ...
          // It is possible that not every contouri pixel would have been
          // expanded at the first step.
